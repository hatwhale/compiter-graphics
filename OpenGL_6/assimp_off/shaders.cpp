...
CShaderProgram spMain, spColor, spShadowMapper, spShadowMapRender, spSkybox;
...
bool PrepareShaderPrograms()
{
	...
	string sShaderFileNames[] = {"main_shader.vert", "main_shader.frag", "dirLight.frag", "pointLight.frag",
		"color.vert", "color.frag", "spotLight.frag", "shadowMapper.vert", "shadowMapper.frag",
		"shadowMapRender.vert", "shadowMapRender.frag", "shadows.frag", "skybox.vert", "skybox.frag"
	};
	...
	spMain.AddShaderToProgram(&shShaders[11]);
	spMain.LinkProgram();
	...
	if(!spColor.LinkProgram()) return false;

	spShadowMapper.CreateProgram();
	spShadowMapper.AddShaderToProgram(&shShaders[7]);
	spShadowMapper.AddShaderToProgram(&shShaders[8]);
	if (!spShadowMapper.LinkProgram())return false;

	spShadowMapRender.CreateProgram();
	spShadowMapRender.AddShaderToProgram(&shShaders[9]);
	spShadowMapRender.AddShaderToProgram(&shShaders[10]);
	if (!spShadowMapRender.LinkProgram())return false;

	spSkybox.CreateProgram();
	spSkybox.AddShaderToProgram(&shShaders[12]);
	spSkybox.AddShaderToProgram(&shShaders[13]);
